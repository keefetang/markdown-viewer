...
**Content negotiation:** Both `/` and `/:id` support two ways to request raw content: the `Accept: text/markdown` header (for programmatic clients) or the `?format=md` query parameter (for browser address bars, links, and docs). When either is used, `/` returns a markdown description of the tool and `/:id` returns the raw markdown content. Additionally, `?format=html` on `/:id` returns a standalone rendered HTML document with inline styles (same as the browser's Export → HTML). Private sessions require `X-Edit-Token` for content negotiation too. This enables CLI tools (`curl -H 'Accept: text/markdown'`), AI agents, and direct browser links (`https://markdown.pentagram.me/:id?format=md`) to fetch content without parsing HTML.
